...
* Redis 기반 인증 캐시, QR 토큰 중앙화, 병원 조회 글로벌 캐시 적용
...
| 인증 병목 완화     | Redis 인증 캐시 적용으로 평균 지연 시간 `38ms → 4ms`       |
| 처리량 개선       | 인증 구간 부하 테스트 기준 처리량 `8.9/sec → 967.1/sec`    |
...
| 인증 구간 DB 조회 감소 | JWT 인증 후 매 요청마다 사용자 DB 조회 발생 | Redis Look-Aside Cache 적용            | 평균 지연 `38ms → 4ms`, 처리량 `8.9/sec → 967.1/sec` |
...
JWT 인증 이후에도 `loadUserByUsername()`을 통해 매 요청마다 사용자 정보를 DB에서 다시 조회하고 있었음.
JMeter 부하 테스트 결과, 단순 인증 확인 요청에서도 커넥션 풀 경합으로 최대 지연 시간이 `547ms`까지 증가하는 문제가 확인됨.

이를 해결하기 위해 `UserDetailsService`에 Redis 기반 Look-Aside Cache를 적용함.
사용자 정보 수정, 비밀번호 변경, 회원 탈퇴 시에는 email key 기준으로 수동 eviction을 적용해 캐시 정합성을 유지함.

개선 결과 최대 지연 시간은 `547ms → 8ms`, 평균 지연 시간은 `38ms → 4ms`로 감소했고, 처리량은 `8.9/sec → 967.1/sec`로 증가함.
